...
const Hero = () => {
  ...
  useEffect(() => {
    ...
    for (let i = 1; i <= frameCount; i++) {
      ...
      const img = new Image();
      // Load from public directory for production-safe routing in Vite
      img.src = `/hero-video/ezgif-frame-${num}.png`;
  ...
  return (
    <section ref={containerRef} className="relative h-[400vh] w-full bg-black">
      ...
      >
        ...
        <img 
          ref={topLayerRef}
          src="/hero-video/ezgif-frame-240.png" 
        ...
        <div className="absolute bottom-0 left-0 w-full z-[10] px-8 md:px-16 pb-12 flex flex-col pointer-events-auto">
          ...
          {/* Top Row: 3 Columns */}
          <div className="flex flex-col md:flex-row justify-between items-start md:items-end w-full mb-8 max-w-[1800px] mx-auto">
          ...
          {/* BOTTOM ROW */}
          <div className="w-full max-w-[1800px] mx-auto flex flex-col md:flex-row justify-between items-start md:items-center pt-8 border-t border-[rgba(255,255,255,0.05)] gap-6 md:gap-0">
            
            {/* Left Side Socials */}
            <div className="flex flex-wrap items-center gap-6 md:gap-12 md:mb-0">
              {[
                { name: 'GITHUB', url: 'https://github.com/AND-adi' },
                { name: 'LINKEDIN', url: 'https://www.linkedin.com/in/aditya-narayan-dash-548a0835b/' },
                { name: 'EMAIL', url: 'mailto:adityanarayandash6006@gmail.com' }
              ].map((social) => (
                <a 
                  key={social.name} 
                  href={social.url}
            ...
            <div className="text-[rgba(255,255,255,0.5)] text-[10px] uppercase tracking-[1px] text-left md:text-right">
              &copy; 2026 AND.WEB. ALL RIGHTS RESERVED.
            </div>
